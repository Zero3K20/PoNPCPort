...
int Okimono::o_data[14] = {
    537011713, 481781268, 481764869, 1346823684, 1348920836,
    1350493700, 1362110474, 1362126858, 134440712, 134440714,
    134440716, 134440718, 1208002620, 134358529
};

Okimono::Okimono(int n) : oki_id(n) {}
...
void Okimono::Move() {
    ...
    if (drop_time == 1) {
        ...
        if (move_cnt % 4 == 0) {
            n = CpCanvas::AtaNo(0);
            CpCanvas::ata[n]->Set(85, 2 + flp, move_cnt % 12 / 4, flp, -2);
        }
        n = move_cnt % 15 / 5;
        if (move_cnt % 45 < 15 &&
            CpCanvas::ene[n]->chara_no != 12 &&
            CpCanvas::ene[n]->chara_no != 13 &&
            CpCanvas::ene[n]->chara_no != 32 &&
            CpCanvas::ene[n]->move_flg == 0 &&
            CpCanvas::ene[n]->tmp_x == CpCanvas::ene[n]->pos_x &&
            CpCanvas::ene[n]->tmp_y == CpCanvas::ene[n]->pos_y) {
        ...
        if (move_cnt % 4 == 0) {
            n = CpCanvas::AtaNo(0);
            CpCanvas::ata[n]->Set(86, flp * 5, move_cnt % 12 / 4, flp ^ 1, -2);
        }
        n = move_cnt % 15 / 5;
        if (move_cnt % 45 < 15 &&
            CpCanvas::ene[n]->chara_no != 12 &&
    ...
    } else if (syu > 7 && syu < 12 && move_cnt > 60) {
        pos_dx = 2;
        int n = CpCanvas::AtaNo(0);
        CpCanvas::ata[n]->Set(125, pos_x, pos_y, flp, -2);
        CpCanvas::ata[n]->pow       = pow;
        CpCanvas::ata[n]->chara_flg = 1 - flp;
        if (waza_syu == 0) {
            move_cnt = -static_cast<int>((CpCanvas::rand() & 0x7FFFFFFF) % 30);
        } else {
            move_cnt = 40;
            waza_syu = 0;
        }
...
int Okimono::PaneRan(int n) {
    ...
    for (int n2 = 0; n2 < 18; ++n2) {
        if (CpCanvas::panel[n2 / 6][n2 % 6].jin != n ||
            CpCanvas::panel[n2 / 6][n2 % 6].jou <= 0) continue;
        nArray[n3] = n2;
        ++n3;
    }
    if (n3 == 0) return 100;

    int n2 = static_cast<int>((CpCanvas::rand() & 0x7FFFFFFF) % (3 + CpCanvas::ene_cnt));
    if (n2 < 3) {
        if (n == 0 && n2 == 0) {
        ...
        return nArray[(CpCanvas::rand() & 0x7FFFFFFF) % n3];
    }
    return nArray[(CpCanvas::rand() & 0x7FFFFFFF) % n3];
}
...
void Okimono::Hit(int n, int n2) {
    ...
    if (CpCanvas::panel[pos_y][pos_x].jou == 7) {
        n = (n + 1) / 2;
    }
    hp -= n;
    if (n2 != 0 && syu < 12) hp = 0;
    if (hp <= 0) {
        ...
        if (syu < 12) {
            ...
            CpCanvas::ata[n3]->Set(4, pos_x, 0, 0, 3);
            n3 = CpCanvas::AtaNo(0);
            CpCanvas::ata[n3]->Set(4, pos_x, 1, 0, 3);
            n3 = CpCanvas::AtaNo(0);
            CpCanvas::ata[n3]->Set(4, pos_x, 2, 0, 3);
            n3 = CpCanvas::AtaNo(0);
...
void Okimono::NextOff() {
    if (oki_id % 2 == 0) {
        ...
        CpCanvas::oki[oki_id - 1]->next_off =
            (CpCanvas::oki[oki_id - 1]->on == 0) ? oki_id : oki_id - 1;
    }
}
...
void Okimono::Draw() {
    ...
    if (-2 == CpCanvas::Ani(ani_id, ani_pt, ani_cnt,
        ...
        ani_cnt = 0;
        CpCanvas::Ani(ani_id, ani_pt, ani_cnt,
                      pos_x, pos_y, pos_dx, pos_dy, flp);
    }
    pos_dx = 0;
    pos_dy = 0;
