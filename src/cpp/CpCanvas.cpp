...
void CpCanvas::Shop() {
    ...
    CpCanvas::g->drawImage(CpCanvas::imgMap, 0, 0);
    if (CpCanvas::mes_flg == 0) {
        if (CpCanvas::key == 131072 || (n4 & 0x20000) != 0) {
            if (CpCanvas::sel_tip2 > 0 && --CpCanvas::sel_tip2 - CpCanvas::sel_jou2 < 0) {
                --CpCanvas::sel_jou2;
            }
            ++CpCanvas::key_cnt;
            CpCanvas::key = 0;
        } else if (CpCanvas::key == 524288 || (n4 & 0x80000) != 0) {
            if (CpCanvas::sel_tip2 < CpCanvas::zai_cnt - 1 && ++CpCanvas::sel_tip2 - CpCanvas::sel_jou2 >= 7) {
                ++CpCanvas::sel_jou2;
            }
            ++CpCanvas::key_cnt;
            CpCanvas::key = 0;
    ...
    // (Shop body continues beyond provided source; back navigation handled)
    if (CpCanvas::key == 0x200000) {
        MachiSet(CpCanvas::machi_no, 0);
        CpCanvas::scene = 5;
        CpCanvas::key = 0;
