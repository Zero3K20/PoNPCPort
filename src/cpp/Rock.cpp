...
int* Rock::ani_id        = nullptr;
int* Rock::ani_pt        = nullptr;
int  Rock::ani_max       = 0;
int**Rock::move          = nullptr;
int  Rock::waza_flg      = 0;
...
static int s_pow[4]     = {};
static int s_ani_id[2]  = {};
static int s_ani_pt[2]  = {};
static int s_move_rows[22][7] = {};
static int* s_move_ptrs[22]   = {};

Rock::Rock() {
    // Static array storage – initialise once
    if (Rock::pow == nullptr) {
        Rock::pow    = s_pow;
        Rock::ani_id = s_ani_id;
        Rock::ani_pt = s_ani_pt;
        Rock::move   = s_move_ptrs;
        for (int i = 0; i < 22; ++i) Rock::move[i] = s_move_rows[i];
    }
}

void Rock::init() {
    for (int n = 0; n < 9; ++n) {
        tip[n] = CpCanvas::tip[0];
    }
    t_ani_cnt  = 0;
    tmp_s_wait = 0;
    tmp_r_suu  = 0;
...
void Rock::Move() {
    ...
    if (move_flg > 5) {
        ...
        bool loopOk =
            zoku != 2 && CpCanvas::panel[Rock::pos_y][Rock::pos_x].jou == 5 &&
            ((move_flg - 5 == 1 && Check(-1, 0, 2) == 0) ||
             (move_flg - 5 == 3 && Check( 1, 0, 2) == 0) ||
             (move_flg - 5 == 2 && Check(0,-1, 2) == 0) ||
             (move_flg - 5 == 4 && Check(0, 1, 2) == 0));
        move_flg = loopOk ? (move_flg -= 5) : 0;
    }
    ...
    if (waza_flg >= 0 && ani_wait_cnt == 0) {
        if (waza_wait == 0) {
            ...
            if (n != 0) {
                ...
                CpCanvas::stop_ch   = 0;
                for (int i = 1; i < 6; ++i) {
                    if (tip[Rock::tip_no + i]->syu >= 4) continue;
                    CpCanvas::stop_name = tip[Rock::tip_no + i]->name;
                    break;
                }
            }
        }
...
void Rock::PaneCh() {
    ...
    if (CpCanvas::panel[Rock::pos_y][Rock::pos_x].bom_pow > 0) {
        Hit(1, CpCanvas::panel[Rock::pos_y][Rock::pos_x].bom_pow, 0);
        EffSet(4);
...
void Rock::TipDraw() {
    ...
    for (n = 0; n < tip_no + 1 && joutai != 21; ++n) {
        ...
        CpCanvas::tip[tip[n]->tip_id]->Draw(n2, n3, 0, true);
    }
    ...
    if (tip[Rock::tip_no]->syu < 3) {
        ...
        CpCanvas::g->setColor(Graphics::getColorOfRGB(255, 255, 0));
        CpCanvas::strDraw(" " + std::to_string(tip[Rock::tip_no]->pow),
                          5 + CpCanvas::f->stringWidth(tip[Rock::tip_no]->name), 215);
        if (pow[0] != 0) {
            CpCanvas::strDraw(
                "+" + std::to_string(pow[0]),
                5 + CpCanvas::f->stringWidth(
                    tip[Rock::tip_no]->name + " " +
                    std::to_string(tip[Rock::tip_no]->pow)),
                215);
        }
    }
}
...
void Rock::Draw(int n) {
    ...
    for (int i = 0; i < ani_max; ++i) {
        if (-2 != CpCanvas::Ani(ani_id[i], ani_pt[i], ani_cnt,
                                 pos_x, pos_y + n2,
                                 dx + tmp_ani_cnt % 3, dy, 1)) continue;
        if (ani_pt[0] != 0) {
            AniSet(0);
        } else {
            ani_cnt = 0;
        }
        CpCanvas::Ani(ani_id[i], ani_pt[i], ani_cnt,
                      pos_x, pos_y + n2,
                      dx + tmp_ani_cnt % 3, dy, 1);
    }
    t_ani_cnt = ani_cnt;
